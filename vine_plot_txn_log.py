...
class TxnPlot:
    ...
    def bh(self, fig, slots, left, width, color, **kwargs):
        ...
        args = { "left": left, "width": width, "color": color, "linewidth": 0.5, "align": "edge", "edgecolor": edge_color }
        args.update(kwargs)
...
class TxnPlotWorkers(TxnPlot):
    ...
    def plot_by_worker_dask(self, fig, worker_tasks, worker_times, worker_xfers, base_slot):
        ...
        self.bh(fig, base_slot, cache_updates["start_time"], cache_updates["wall_time"], 
                self.legend_dask["data transfers"], height=slot_height, type="cu_dask")

        # every following task's bar slot add 1
        worker_tasks["slot"] += 1

        # plot the green bar: tasks executing
        self.bh(fig, retrieved_tasks["slot"], retrieved_tasks["time_worker_start"], 
                retrieved_tasks["time_worker_end"] - retrieved_tasks["time_worker_start"], 
                self.legend_dask["tasks executing"], height=slot_height, type="te_dask")
    ...
    def plot(self, fig, manager):
        ...
        for wid in by_min_connects:
            ...
            if worker is not None:
                if args.dask_colors:
                    base_slot = self.plot_by_worker_dask(fig, worker, worker_times, worker_xfers, base_slot)
                else:
